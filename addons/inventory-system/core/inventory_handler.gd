...
func _ready():
	...
	if not Engine.is_editor_hint() and recreate_slots_on_ready:
		...
		inventory._load_slots()
		hotbar.slots_in_hot_bar = slots_in_hot_bar
...
func drop(item : InventoryItem, amount := 1) -> bool:
	if item.properties.has("dropped_item_model"):
		...
		#for i in amount:
		_instantiate_dropped_item(dropped_item,item,amount)
		return true
	else:
		return false
...
func pick_to_inventory(dropped_item, inventory := self.inventory):
	if not dropped_item is PickUpItem:
		return false
	if not dropped_item.is_pickable:
		return false
	var item = dropped_item.item
	var amount = dropped_item.amount
	if add_to_inventory(inventory, item, amount) == 0:
...
func _instantiate_dropped_item(dropped_item : PackedScene,item : InventoryItem, amount: int):
