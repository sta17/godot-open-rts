...
class Actions:
	...
	const MoveAndCollect = preload("res://source/match/units/actions/MoveAndCollectItem.gd")
...
func _navigate_selected_units_towards_unit(target_unit):
	...
	for unit in get_tree().get_nodes_in_group("selected_units"):
		if not unit.is_in_group("controlled_units"):
			continue
		if Actions.MoveAndCollect.is_applicable(unit, target_unit):
			unit.action = Actions.MoveAndCollect.new(target_unit)
			units_navigated += 1
		elif Actions.CollectingResourcesSequentially.is_applicable(unit, target_unit):
